...
class BenchmarkReportService {
    ...
    async reportBenchmarkResultPromise(request) {
        ...
        try {
            ...
            if (request.getResult()?.getFinished()) {
                ...
                await db.commit();
                app_1.notifier.notifyBenchmarkJobFinished(job, db);
            }
            else {
    ...
    async saveAsFinished(job, request, db) {
        ...
        WHERE id = ?
        LIMIT 1
    `, [benchmark_job_pb_1.BenchmarkJob.Status.FINISHED, result?.getScoreBreakdown()?.getRaw(), result?.getScoreBreakdown()?.getDeduction(), result?.getPassed(), result?.getReason(), markedAt, request.getJobId()]);
    }
