...
const poolPromise = promise_mysql_1.default.createPool(exports.dbinfo);
let pool = null;
exports.getDB = async () => {
    if (pool == null) {
...
function getNotificationsResource(notifications) {
    return notifications.map(notification => {
        const notificationResource = notification_pb_1.Notification.deserializeBinary(Buffer.from(notification.encoded_message, 'base64'));
        notificationResource.setId(notification.id);
        notificationResource.setCreatedAt(notification.created_at ? exports.convertDateToTimestamp(notification.created_at) : null);
...
exports.app.put("/api/admin/clarifications/:id", async (req, res, next) => {
    ...
    try {
        ...
        const [team] = await db.query('SELECT * FROM `teams` WHERE `id` = ? LIMIT 1', [clar.team_id]);
        await db.commit();
        exports.notifier.notifyClarificationAnswered(clar, db, wasAnswered && wasDisclosed == clar.disclosed);
